...
// validatePreferences checks that every wire name in the resource's
// ownership entries resolves to a field the generator emits.
//
// A schema change that drops a field the artifact still records must stop
// generation rather than emit a name the struct no longer has. Fields are
// matched after removals and adds, so the names must match the struct the
	...
	for _, key := range slices.Sorted(maps.Keys(prefs)) {
		...
		for _, excluded := range prefs[key].UOSExcludes {
			if !slices.Contains(prefs[key].Owns, excluded) {
				return fmt.Errorf("%s preference %q: uos_pins names %q, which is not in owns. "+
					"A pin says the console takes a field the mode owns elsewhere, so one that "+
					"names an unowned field describes nothing -- re-measure both harnesses",
