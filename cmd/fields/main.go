...
// hand-written client owns, because what this SDK publishes for them is not
// the generic shape. The generator never publishes those names. The value
// says whether the hand-written method calls the generated implementation,
// which is then emitted unexported, or issues its own request, in which case
// the generator emits nothing for that verb.
var handWrittenCRUD = map[string]map[string]bool{
	"Client": {
		"List": true, // publishes the list endpoint's query pass-through
		// A client is forgotten through stamgr (DeleteClientByMAC); no REST
		// delete of a client object is published.
		"Delete": false,
	},
	"ContentFiltering": {
		"List": true, // publishes the list endpoint's query pass-through
	...
	"Device": {
		// The device read endpoint is keyed by MAC, so the published
		// GetDevice filters the list by id and the generated by-MAC read
		// stays internal (GetDeviceByMAC, rereadDevice, UpdateDevice).
		"Get": true,
		// UpdateDevice writes only the difference from the stored object.
		"Update": false,
	},
	"Nat": {
		"List": true, // publishes the list endpoint's query pass-through
	...
	"Network": {
		"List": true, // published with a parameter the implementation ignores
		// DeleteNetwork sends the network name in the request body.
		"Delete": false,
	},
	"NetworkMembersGroup": {
		// Published as ListNetworkMembersGroups, reads the per-id path
		// instead of filtering the list, clears the id on create, and
		// deletes with no request body.
		"List":   false,
		"Get":    false,
		"Create": false,
		"Update": false,
...
// verb, rather than leaving that name to the hand-written client.
func (r *ResourceInfo) Exports(verb string) bool {
	_, handWritten := handWrittenCRUD[r.StructName][verb]
	return !handWritten
}

// Emits reports whether the generator emits the verb at all: a hand-written
// method that issues its own request needs no generated implementation.
func (r *ResourceInfo) Emits(verb string) bool {
	return r.Exports(verb) || handWrittenCRUD[r.StructName][verb]
}

// Method is the name the generator emits for a CRUD verb.
func (r *ResourceInfo) Method(verb string) string {
	if r.Exports(verb) {
		return verb + r.StructName
	}
	return strings.ToLower(verb[:1]) + verb[1:] + r.StructName
}
