...
func TestOrdinaryCIIsOfflineAndBlocking(t *testing.T) {
	...
	assert.Contains(t, ci, `go build -o "$RUNNER_TEMP/apicompat"`)
	assert.Contains(t, ci, `"$RUNNER_TEMP/apicompat"`)
...
func TestSchemaUpdateWorkflowValidatesBeforePrivilege(t *testing.T) {
	...
	assert.Contains(t, workflow, "  pull-requests: read")
	...
	assert.Contains(t, workflow, "pending_pr_count")
	assert.Contains(t, workflow, "merge-base --is-ancestor origin/main")
	assert.Contains(t, workflow, `.head.repo.full_name == $repo`)
	assert.Contains(t, workflow, "pending_generated_digest")
	assert.Contains(t, workflow, "expected-generated-tree-digest")
	assert.Contains(t, workflow, "api-compatibility: (compatible|breaking)")
	assert.Contains(t, workflow, "desired_auto_merge")
	assert.Contains(t, workflow, "current_auto_merge")
	assert.Contains(t, workflow, `"$desired_auto_merge" == "$current_auto_merge"`)
	baseGate := strings.Index(workflow, "merge-base --is-ancestor origin/main")
	prGate := strings.Index(workflow, `if [[ "$pending_pr_count" -eq 1 ]]`)
	digestGate := strings.Index(workflow, `[[ "$expected_digest" == "$pending_generated_digest" ]]`)
	autoMergeGate := strings.Index(workflow, `"$desired_auto_merge" == "$current_auto_merge"`)
	pendingSHA := strings.Index(workflow, "automation_sha=$(git show")
	require.NotEqual(t, -1, baseGate)
	require.NotEqual(t, -1, prGate)
	require.NotEqual(t, -1, digestGate)
	require.NotEqual(t, -1, autoMergeGate)
	require.NotEqual(t, -1, pendingSHA)
	assert.Less(t, baseGate, prGate, "a stale-base branch must not suppress regeneration")
	assert.Less(t, prGate, digestGate, "a branch without one exact open PR must not suppress regeneration")
	assert.Less(t, digestGate, autoMergeGate, "missing or stale PR provenance must not suppress regeneration")
	assert.Less(t, autoMergeGate, pendingSHA, "incorrect pending auto-merge state must be reconciled")
	...
	assert.Contains(t, workflow, `go build -o "$RUNNER_TEMP/apicompat"`)
	assert.Contains(t, workflow, `"$RUNNER_TEMP/apicompat"`)
	...
	assert.Contains(t, workflow, "autoMergeRequest")
	assert.Contains(t, workflow, "--disable-auto")
	assert.Contains(t, workflow, "pr_number=")
