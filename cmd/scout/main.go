...
func run(args []string, stderr io.Writer) int {
	...
	if *targetPath == "" || *scenarioPath == "" || *structuralPath == "" || *semanticIDsPath == "" ||
		*captureLockPath == "" || *catalogOutput == "" || *receiptOutput == "" {
		fmt.Fprintln(stderr, "target-profile, scenario, structural, semantic-ids, capture-lock, catalog-output, and receipt-output are required")
		return 2
	}
	...
	if *responsePath != "" {
		...
		measuredTargetFingerprint = target.ControllerFingerprint
	}
...
func observeLive(ctx context.Context, scenario scout.Scenario) ([]byte, error) {
	...
	return client.ListDNSRecordRaw(ctx, envDefault("UNIFI_SITE", "default"))
}
