...
func declaredWireFields(d any) map[string]reflect.StructField {
	...
	// The recorded Index must be the full path from the outer type:
	// StructField.Index from a recursive walk is relative to the embedded
	// type, and FieldByIndex applied to the outer value would then probe
	// the wrong field.
...
func encoderEmits(d any, field reflect.StructField, wire string) bool {
	...
	// FieldByIndexErr rather than FieldByIndex: a path through a nil
	// embedded pointer is unreachable, not a panic.
	target, err := probe.Elem().FieldByIndexErr(field.Index)
	if err != nil || !target.CanSet() || !setNonZero(target) {
		return false
	}
