# Unifi Go SDK [![Go Reference](https://pkg.go.dev/badge/github.com/ubiquiti-community/go-unifi.svg)](https://pkg.go.dev/github.com/ubiquiti-community/go-unifi)
...
[ARCHITECTURE.md](ARCHITECTURE.md) describes the whole pipeline — capture,
locked generation, measured overrides, and the release runbook for a new
controller version.
...
remains offline. See [schemas/README.md](schemas/README.md) for the capture
procedure.
...
build. Emulated devices need the external herder binary: point
`UNIFI_TEST_HERDER_BIN` at a release of
[unifi-emu](https://github.com/jamesbraid/unifi-emu) (its install-herder
action does this in CI); without it the suite runs controller-only. CI runs
this gate on every schema change.
